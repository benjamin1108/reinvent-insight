...
## 标题提取/生成规则
1. **英文标题 (title_en)**：
   - 优先从{content_type}内容中提取原始的英文标题
   - 如果{content_type}中没有明确的英文标题，则基于内容生成一个简洁、专业的英文标题
   - 英文标题应该准确反映文档的核心主题
...
1. **产品发布会/Keynote**（如 AWS re:Invent、Apple WWDC、Google I/O）
   ...
   - **章节设计要求**：
     * ✅ **章节策略**：根据章节内容的属性，严格匹配以下对应的标题风格：
       - **产品发布章节** → 使用"具体产品名 + 版本号 + 核心亮点"
       - **客户案例章节** → 使用"公司名 + 应用场景 + 关键成果"
       - **理念讲解章节** → **拒绝宏大叙事，采用"接地气表达 + 实效洞察"**。
         *请根据内容从以下几种模式中灵活/随机选择一种：*
         1. **口语+真相**（如：别发火：发脾气是本能，压下去是本事）
         2. **动作+收益**（如：关掉手机：专注一小时，顶你磨洋工一天）
         3. **比喻+场景**（如：烂尾楼心态：钱都花了，非得把这顿难吃的饭吃完？）
         4. **吐槽+纠正**（如：瞎忙活：感动了自己，却没有任何产出）
...
**你是导演，不是剪辑师**。你的任务是重新设计内容的叙事结构，而非照搬原文顺序。

1. **钩子开场**：
   - 第一章严禁背景介绍或历史铺垫
   - 必须是一个具体的、具有冲击力的场景、结论或数据
   - 让读者在前 30 秒内决定"这篇文章值得读下去"

2. **权重失衡**（打破等分焦虑）：
   - 根据素材的含金量，手动分配章节权重
   - 重要的内容写 5000 字，次要的写 500 字
   - 不要搞平均主义，不要每章都是 3 个子章节
   - 核心章节可以有 5-7 个子章节，边缘章节可以只有 1-2 个
...
2. **章节聚合原则**（防止碎片化）：
   - ✅ **强制合并条件**（满足任一即应合并）：
     * 两个章节讨论同一技术组件的不同侧面（如"物理层"和"带宽分配"应合并）
     * 章节预计字数 < 2500 字，且与相邻章节主题高度相关
     * 标题使用"（上）/（下）"或"Part 1/Part 2"的人为拆分
   - ❌ **禁止拆分条件**：
     * 不要为了凑章节数把一个完整论点拆成多章
     * 同一主题的"问题描述"和"解决方案"不应分开成两章

3. **章节排序原则**（技术深度分析类内容）：
   - 核心能力优先：先完整介绍主要对象的核心能力
   - 系统构成次之：再讲解其系统级组成（硬件→网络→软件）
   - 挑战与局限：放在解决方案之前，而非之后
   - 前瞻展望靠后：未来规划、下一代产品放在倒数第 2-3 章
   - ❌ **禁止**：刚介绍完 V1 核心规格，就跳到 V2 前瞻，再回来讲 V1 细节
...
**跨章节概念归属表**（防止隐性重复的核心机制）：

在生成 chapters 数组前，必须先识别文章涉及的所有核心概念，并为每个概念指定**唯一归属章节**：
- 概念归属章节负责深入讲解该概念的完整逻辑
- 后续章节如需提及，仅允许**引用式提及**（如"正如第X章所述"），禁止重新展开

示例：
```
核心概念归属：
- "供应链金融/股权返利" → 第10章独占，第11章禁止重述机制
- "风冷 vs 液冷 TCO" → 第4章独占，第19章仅讲建设速度
- "MoE 对网络的影响" → 第7章独占，第15章禁止重复
```
...
{{{{
  ...
  "chapters": [
    {{{{
      ...
      "content_guidance": "[自然语言描述该章节的内容范围、重点和注意事项]"
    }}}},
    ...
  ]
}}}}
...
**子章节规划要求**：
- **数量弹性**：根据内容含金量灵活决定，核心章节 3-7 个子章节，边缘章节 1-2 个即可
- **key_points**：每个子章节列出 2-3 个必须覆盖的核心论点
- **subtitle**：子章节标题要具体明确，让读者一看就知道讲什么
- **权重分配**：不要强求每章都是相同数量的子章节，根据内容实际情况灵活调整
- **反平均主义**：如果某个主题特别重要，可以设置更多子章节深入展开；如果某个主题只是过渡，减少子章节数量

## 特殊格式要求
1. **纯 JSON 输出**：只输出 JSON，不要输出任何 Markdown 内容
2. **引言（introduction 字段）**：
   - 200-400字，激发读者阅读兴趣，准确反映整体内容
...
3. **章节数量**：根据原文内容量和复杂度自适应生成
   - **内容丰富**（如长篇报告、深度分析）：建议 12-20 个章节
   - **内容适中**（如标准文章、演讲）：建议 6-12 个章节
   - **内容精简**（如短文、简报）：建议 3-6 个章节
4. **纯文本限制**：章节标题必须是纯文本，**严禁**包含 Markdown 链接格式或方括号 `[]()【】「」`
5. **格式严格性**：不要添加任何额外的解释或章节内容，JSON 之后不要输出任何内容

## 输出格式

**只输出 JSON，不要输出任何 Markdown 内容**

```json
{{{{
  "title_en": "[提取或生成的英文标题]",
  "title_cn": "[生成的中文标题]",
  "introduction": "[200-400字的引言，激发读者兴趣，准确反映整体内容，必须自然地透出内容来源]",
  "chapters": [
    {{{{
      "index": 1,
      "title": "[第一章标题]",
...
---
**重要提醒**：
1. **只输出 JSON，不要输出任何 Markdown 内容**（标题、引言、目录都在 JSON 中）
2. **章节框架是并发生成的关键**：
   - `subsections`: 每章至少 3 个子章节，每个子章节明确 key_points
   - `must_include`: 必须包含的内容，避免遗漏
   - `must_exclude`: **强制负向约束** - 绝对禁止包含的内容，这是防止内容撞车的最强防火墙
   - `content_guidance`: 用自然语言描述内容范围和重点，而非机械标签
3. **subsections 是核心约束**：AI 会严格按照子章节结构展开，每个子章节覆盖所有 key_points
4. **introduction 字段**：200-400字，融入内容来源信息
5. 确保 JSON 格式完全合法，可以被标准解析器解析
6. **禁止在 JSON 之后输出任何内容**
"""
