...
class VisualInterpretationWorker:
    ...
    async def _run_chunked_mode(self) -> str:
        ...
        # 5. 生成静态 Footer（版权信息 + 脚本）
        await self._log("正在生成页面结尾...", progress=85)
        footer_html = self._generate_static_footer(metadata)
    ...
    def _generate_static_footer(self, metadata: Dict) -> str:
        """生成静态 Footer（版权信息 + 必要脚本）
        
        不再使用 AI 生成，直接使用硬编码模板。
        """
        ...
        <!-- Footer 版权信息 -->
        <footer class="mt-16 pb-8 relative z-10">
            <div class="text-center">
                <p class="text-zinc-600 text-xs font-mono">
                    Generated By reInvent Insight • {generation_date}
                </p>
            </div>
        </footer>
    </div>
