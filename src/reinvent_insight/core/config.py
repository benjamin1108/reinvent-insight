...
# 浏览器启动超时（秒）
VISUAL_SCREENSHOT_BROWSER_TIMEOUT = int(os.getenv("VISUAL_SCREENSHOT_BROWSER_TIMEOUT", "90"))
...
# 是否脱敏敏感信息
MODEL_OBSERVABILITY_MASK_SENSITIVE = os.getenv("MODEL_OBSERVABILITY_MASK_SENSITIVE", "true").lower() == "true"

# 提示词最大记录长度
MODEL_OBSERVABILITY_MAX_PROMPT_LENGTH = int(os.getenv("MODEL_OBSERVABILITY_MAX_PROMPT_LENGTH", "2000"))

# 响应内容最大记录长度
MODEL_OBSERVABILITY_MAX_RESPONSE_LENGTH = int(os.getenv("MODEL_OBSERVABILITY_MAX_RESPONSE_LENGTH", "5000"))

# 是否启用业务上下文记录
MODEL_OBSERVABILITY_ENABLE_BUSINESS_CONTEXT = os.getenv("MODEL_OBSERVABILITY_ENABLE_BUSINESS_CONTEXT", "true").lower() == "true"

# 批量写入大小
MODEL_OBSERVABILITY_BATCH_SIZE = int(os.getenv("MODEL_OBSERVABILITY_BATCH_SIZE", "100"))

# 刷新间隔（秒）
MODEL_OBSERVABILITY_FLUSH_INTERVAL = int(os.getenv("MODEL_OBSERVABILITY_FLUSH_INTERVAL", "10"))

# 日志保留天数
MODEL_OBSERVABILITY_RETENTION_DAYS = int(os.getenv("MODEL_OBSERVABILITY_RETENTION_DAYS", "30"))

# 单个文件最大大小（MB）
MODEL_OBSERVABILITY_MAX_FILE_SIZE_MB = int(os.getenv("MODEL_OBSERVABILITY_MAX_FILE_SIZE_MB", "100"))
