...
def update_level_info(metadata: dict) -> dict:
    """
    根据title_en更新level信息
    """
    ...
    # 获取英文标题
    title_en = new_metadata.get('title_en', new_metadata.get('title', ''))
    if not title_en:
        logger.warning("  未找到标题，跳过")
        return None
    
    # 创建VideoMetadata实例来解析level
    video_metadata = VideoMetadata(
        title=title_en,
        upload_date="20231201",  # 这个不重要
        video_url="https://youtube.com/test"  # 这个不重要
    )
    
    # 更新level信息
    old_level = new_metadata.get('level')
    new_level = video_metadata.level
    
    if old_level != new_level:
        new_metadata['level'] = new_level
        logger.info(f"  Level更新: {old_level} -> {new_level}")
        return new_metadata
    else:
        logger.info(f"  Level未变化: {old_level}")
        return None
...
def process_directory(directory: Path):
    ...
    for md_file in md_files:
        ...
        try:
            ...
            # 只处理re:Invent相关的文件
            if not metadata.get('is_reinvent', False):
                logger.info(f"  非re:Invent视频，跳过")
                skipped_count += 1
                continue
            
            # 更新level信息
            new_metadata = update_level_info(metadata)
