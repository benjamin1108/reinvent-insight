...
**重要要求**：
1. **根据原文实际篇幅分章**：
   - 仔细阅读原文，评估每个主题在原文中的讨论深度
   - 如果某个主题在原文中只有1-2句话或一小段讨论，**不要**单独成章
   - 应该将内容稀薄的主题合并到相关章节中
   - 避免为了"故事完整性"而强制创建原文中没有充分讨论的章节
...
**示例说明**：
- 如果某章节原文有3-4段详细讨论 → source_content_amount: "rich", generation_depth: "detailed", target_words: "1200-2000"
- 如果某章节原文只有1段简单提及 → source_content_amount: "sparse", generation_depth: "brief", target_words: "300-600"
- 如果某主题原文几乎没有讨论 → 不要单独成章，合并到相关章节
...
- **建议字数范围**: {target_words} 字
- **建议理由**: {rationale}

**生成要求**：
1. **严格遵守深度建议**：
   - brief（简洁）：300-600字，快速概括核心要点，不展开细节
   - moderate（适度）：600-1200字，适度展开关键论点和案例
   - detailed（详细）：1200-2000字，深入分析并提供详细解读
